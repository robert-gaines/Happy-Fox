...
def GatherTickets(api_key,auth_token):
    #
    tickets = []
    #
    api_key = api_key
...
def FetchVariables(vars):
    ...
    for v in vars:
        ...
        values.append(temp)
        #
    #print("[*]API:",values[0],"[*]AUTH:",values[1],"[*] Month:",values[2])
    #
    api_key = values[0]
    #
    auth_token = values[1]
    ...
    sorted_tickets = []
    #
    tickets = GatherTickets(api_key,auth_token)
    #
    month_index = int(values[2])
    #
...
def GraphicalMain():
    ...
    base = Tk()
    #
    base.title("Monthly Report Generator")
    #
    base.geometry("500x100")
    #
    fields = 'API Key','Authorization Token','Month(#)'
    #
    vars = MakeForm(base,fields)
    #
    Button(base,text='Generate',width=25,command=(lambda: FetchVariables(vars))).pack(side=RIGHT)
    #
    Button(base,text="Quit",width=25,command=base.destroy).pack(side=LEFT)
    #
    base.bind('<Return>',(lambda event: FetchVariables(vars)))
    #
    base.mainloop()
